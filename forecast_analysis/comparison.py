...
class ComparisonView:
    ...
    coverage KPI and retained in ``population_summary``.
    """
...
def _keys_with_forecast(pair_frame: pl.DataFrame) -> pl.DataFrame:
    return _key_frame(
        pair_frame.filter(pl.col("vintage_b_calculation_month").is_not_null())
    )
...
def _coverage_pct(
    ...
    return represented_actual / total_actual * 100
...
def build_source_comparison(
    ...
    *,
    ...
    common_keys = _intersection(tm_available_keys, ml_available_keys)
    tm_only_keys = _difference(tm_available_keys, ml_available_keys)
    ml_only_keys = _difference(ml_available_keys, tm_available_keys)
    ...
    common_metrics_by_source = {
        "tm": calculate_metrics(tm_visible, selected_actual_population),
        "ml": calculate_metrics(ml_visible, selected_actual_population),
    }
    coverage_by_source = {
        "tm": _coverage_pct(tm_available_keys, selected_actual_population),
        "ml": _coverage_pct(ml_available_keys, selected_actual_population),
    }
    metrics_by_source = {
        source: replace(
            common_metrics_by_source[source],
            coverage_pct=coverage_by_source[source],
        )
        for source in COMPARISON_SOURCES
    }
    ...
    return ComparisonView(
        ...
        monthly_performance=build_monthly_performance(
            pl.concat([tm_visible, ml_visible], how="vertical"),
            selected_actual_population,
        ),
        ...
        revision_diagnostics=_source_revision_diagnostics(tm_visible, ml_visible),
